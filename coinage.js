...
function setup() {
  const windowRatio = window.innerWidth/window.innerHeight
    ...
    __canvas = createCanvas(1200,1200)
    SCALE = 1200/H
...
function draw() {
  ...
  const allowHole = false//!IS_VINTAGE && prb(0.5)
  const extraPieces = 2//prb(0.25) ? sample([1, 2]) : 0
  const p = getHighlightPColors()

  rosetteWithBackground(0,0,180, 0, p)
  if (allowHole) rosetteWithBackground(0,0,70, 0, {...p, holeR: 60})
  else if (extraPieces) {
    rosetteWithBackground(0,0,135, 0, 0, p)
    extraPieces > 1 && rosetteWithBackground(0,0,90, 0, 0, p)
  }
